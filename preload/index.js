...
const animationDescription = () => {
  ...
  if (descriptionBackground != undefined) {
    descriptionBackground.innerHTML = descriptions[indexDescription];
    if (indexDescription < descriptions.length - 1) {
      indexDescription++;
    } else {
      indexDescription = 0;
    }
...
window.addEventListener('DOMContentLoaded', () => {
  ...
  // render
  const contentHTML = new DOMParser().parseFromString(content, "text/xml");

...
const descriptions = [
  "Ứng dụng quản lý wifi tiện lợi, dễ dàng sử dụng.",
...
const animationDescription = () => {
  const descriptionBackground = document.getElementsByClassName(
      ...
      profileWifi.list.forEach((value, index) => {
        ...
        wrapper.addEventListener("click", () => {
          handle.get_detail(value).then((detail) => {
            ...
            let data = {};
            data.ssid = detail.find((index) => {
              if (index.key === "SSID name") {
                return index;
              }
            }).value;
            data.security = detail.find((index) => {
              if (index.key === "Authentication") {
                return index;
              }
            }).value;
            data.pass = detail.find((index) => {
              if (index.key === "Key Content") {
                return index;
              }
            })?.value;
...
const renderQR = async (ssid, security, pass, isHidden) => {
  ...
  QRCode.toCanvas(
    qrdom,
    `WIFI:T:${security};S:${ssid};P:${pass};H:${isHidden};;`,
    (error) => {
...
const renderInfoScreen = (data) => {
  ...
  if (infoScreenState === "background") {
    ...
    backgroundDOM.innerHTML = profileScreen(
      data.ssid,
      data.security,
      data.pass
    );
    ...
    backgroundDOM.innerHTML = scanScreen(
      data.ssid,
      signal,
      data.channel,
      data.security,
